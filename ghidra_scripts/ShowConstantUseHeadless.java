...
//
//   This script was modified to be run in headless mode. The "cursor position"
//   has to be supplied by another script via the passed state
// @category Search
...
public class ShowConstantUseHeadless extends HeadlessScript {
  ...
  public void printerLost(Function fun, Address adr) {
    println(
        programName
            + "  "
            + functionName
  ...
  public void printer(Function fun, Address adr, long value) {
    String datAddr = "0x" + Long.toHexString(value);
    String datRep = getStringFromAddress(datAddr);
    println(
        programName
            + "  "
            + functionName
  ...
  public String getStringFromAddress(String addr) {
    try {
      Address genaddr = currentAddress.getAddress(addr);
      Data dat = DataUtilities.getDataAtAddress(currentProgram, genaddr);
      return dat.getDefaultValueRepresentation();
    } catch (Exception e) {
      return new String();
    }
  }
  ...
  public void run() throws Exception {
    ...
    functionName = ((String) this.state.getEnvironmentVar(new String("FunName")));
    parameterName = ((String) this.state.getEnvironmentVar(new String("FunParam")));
    currentLocation = ((ProgramLocation) this.state.getEnvironmentVar(new String("Loc")));
    ...
    try {
      ...
      HashMap<Address, Long> constLocs = null;
      ...
      if (var != null && !var.isConstant()) {
        ...
        if (currentLocation instanceof FunctionParameterFieldLocation) {
          FunctionParameterFieldLocation funcPFL = (FunctionParameterFieldLocation) currentLocation;
  ...
  private Function getReferencedFunction(Address faddr) {
    ...
    if (f == null) {
      Reference[] referencesFrom = currentProgram.getReferenceManager().getReferencesFrom(faddr);
  ...
  private void configureTableColumns(TableChooserDialog tableChooserDialog) {
    ...
    StringColumnDisplay constColumn =
        new StringColumnDisplay() {
          @Override
    ...
    //
    StringColumnDisplay addrPreviewColumn =
        new StringColumnDisplay() {
          @Override
          public String getColumnName() {
            return "Addr Preview";
    ...
    StringColumnDisplay funcColumn =
        new StringColumnDisplay() {
          @Override
          public String getColumnName() {
            return "Func Name";
    ...
    StringColumnDisplay noteColumn =
        new StringColumnDisplay() {
          @Override
          public String getColumnName() {
            return "Note";
  ...
  /**
   * Sample execution task Execution class called whenever the execute button in the table is
   * called. NOTE: the execute button is not setup, so this is just and example
   *
   * <p>Useful if you are back tracking constants for malloc or calloc Runs another script that will
   * create a structure on the return variable of calloc/malloc. It pulls a little trick when
   * calling the CreateStructure script by creating an artificial ScriptState. This is a useful
   * technique for other scripts as well.
   *
   * @return
   */
  @SuppressWarnings("unused")
  ...
  /**
   * Try to locate the Varnode that represents the variable in the listing or decompiler. In the
   * decompiler this could be a local/parameter at any point in the decompiler. In the listing, it
   * must be a parameter variable.
   *
   * @return
   */
  private Varnode getVarnodeLocation() {
    ...
    if (currentLocation instanceof DecompilerLocation) {
      ...
      println(String.valueOf(currentLocation.getRow()));
  ...
  private void addConstants(
      TableChooserDialog tableChooserDialog, HashMap<Address, Long> constLocs) {
    Set<Address> keys;
  ...
  private void addConstantProblem(
      TableChooserDialog tableChooserDialog, Address refAddr, String problem) {
  ...
  public class FunctionParamUse {
    ...
    public FunctionParamUse(
        String name, Address addr, int i, Varnode varnode, ArrayList<PcodeOp> defUseList) {
      this.name = name;
      this.addr = addr;
      this.paramIndex = i;
  ...
  /**
   * Backtrack to a constant given a varnode within a decompiled function This isn't useful for
   * functions that can't be decompiled
   *
   * @param var varnode that represents a variable in a decompilation
   * @param tableChooserDialog - accumulate entries. Don't like passing it, but this way the user
   *     gets immediate feedback as locations are found
   * @return a map of Addresses->constants (constants could be NULL)
   * @throws CancelledException
   */
  private HashMap<Address, Long> backtrackToConstant(
      Varnode var, TableChooserDialog tableChooserDialog) throws CancelledException {
  ...
  /**
   * Backtrack to a constant given a start position of a parameter of a given function Useful if you
   * want to start from a function paramter.
   *
   ...
   * @throws CancelledException
   */
  private HashMap<Address, Long> backtrackParamToConstant(
      Function f, int paramIndex, TableChooserDialog tableChooserDialog) throws CancelledException {
    HashMap<Address, Long> constUse = new HashMap<Address, Long>();
    ...
    funcList.add(
        new FunctionParamUse(
            f.getName(), f.getEntryPoint(), paramIndex, pvnode, new ArrayList<PcodeOp>()));
  ...
  private HashMap<Address, Long> followFunctionParamToConstant(
      ...
      HashMap<Address, Long> constUse,
      TableChooserDialog tableChooserDialog)
      throws CancelledException {
    ...
    while (funcList.size() > 0) {
      ...
      ReferenceIterator referencesTo = currentProgram.getReferenceManager().getReferencesTo(addr);
    ...
    while (ops.hasNext() && !monitor.isCancelled()) {
      ...
      if (pcodeOpAST.getOpcode() == PcodeOp.CALL) {
        ...
        if (parm == null) {
          ...
          String problem =
              "  *** Warning, it appears that function '"
                  + funcVarUse.getName()
                  + "' at "
                  + funcVarUse.getAddress()
        ...
        if (parm.isConstant() || parm.isAddress()) {
          ...
          try {
            ...
            // Address addr = GenericAddress.getAddress(Long.roString(value));
            printer(f, instr.getAddress(), value);
            // println("   " + f.getName() + " " + instr.getAddress() + "  0x" +
            // Long.toHexString(value) + "   " + Long.toString(value));
  ...
  private void followToParam(
      ...
      if (hvar instanceof HighParam) {
        funcList.add(
            new FunctionParamUse(
                ...
                ((HighParam) hvar).getSlot(),
                hvar.getRepresentative(),
                defUseList));
        return;
      }
    ...
    switch (opcode) {
      case PcodeOp.COPY:
        if (def.getInput(0).isConstant()) {
          ...
          try {
            ...
            printer(function, def.getOutput().getPCAddress(), value);
        ...
        followToParam(constUse, defUseList, highFunction, def.getInput(0), funcList, doneSet);
        return;
      case PcodeOp.LOAD:
        if (def.getInput(0).isConstant() && def.getInput(1).isConstant()) {
          ...
          try {
            ...
            constUse.put(remapAddress(funcEntry, def.getOutput().getPCAddress()), offset);
            printer(function, def.getOutput().getPCAddress(), offset);
            // println("   " + function.getName() + "  " +
      ...
      case PcodeOp.MULTIEQUAL:
        ...
        followToParam(constUse, splitUseList, highFunction, def.getInput(1), funcList, doneSet);
        return;
      case PcodeOp.CAST:
        followToParam(constUse, defUseList, highFunction, def.getInput(0), funcList, doneSet);
        return;
